...
# create graphs for visualising the model checking results
  ...
  prob_vs_rp <- ggplot(data=n5,aes(y = prob_sync, x = RP, color=factor(ML), shape=factor(ML)) ) +
    geom_line() +
    geom_point(size=3)+
    theme_bw()+
    theme(legend.position=c(.95,.8), legend.justification = c(0.95,0.85), text = element_text(size=gtext))+
    scale_x_continuous(breaks=c(0,2,4,6,8,10,12,14))+
    scale+
    ggtitle(substitute("Concrete Model Synchronisation (N=5 T=10"~epsilon*"=.1)")) +
    labs( x = "R", y = "Sync. Probability", colour=expression(mu), shape=expression(mu)) 
  
  print(prob_vs_rp)
  ggsave("concrete_n5_t10_e01_prob_vs_rp.pdf", plot=prob_vs_rp, device="pdf", width=gwidth, height=gheight, units = "cm")
